...
import json
import pandas as pd
import yaml  # Nuevo import

datetime_format = "%Y-%m-%d %H:%M:%S.%f"
ruta_log = "log.json" # Ruta del archivo
ruta_validaciones = "validaciones.yml"  # Nuevo: Ruta del archivo de validaciones
...
# Nuevo: Función para cargar validaciones
def cargar_validaciones():
    try:
        with open(ruta_validaciones, 'r') as f:
            return yaml.safe_load(f) or {'validaciones': []}
    except FileNotFoundError:
        return {'validaciones': []}

# Nuevo: Función para aplicar validaciones
def aplicar_validaciones(base, tabla, campo, valor):
    validaciones = cargar_validaciones()
    reglas = [v for v in validaciones['validaciones'] 
              if v.get('base') == base 
              and v.get('tabla') == tabla 
              and v.get('campo') == campo]
    
    errores = []
    
    for regla in reglas:
        # Validación de requerido
        if regla.get('requerido', False) and (valor is None or str(valor).strip() == ''):
            errores.append(f"El campo {campo} es requerido")
            continue  # Si es requerido y está vacío, no validamos nada más
        
        # Validación de formato numérico debe ir PRIMERO
        if regla.get('formato') == 'numerico':
            if not str(valor).isdigit():
                errores.append(f"El campo {campo} debe contener solo números")
                continue  # Si no es numérico, no validamos nada más
        
        # Validación de tipo entero
        if regla.get('tipo') == 'int':
            try:
                int(valor)
                if 'max_digitos' in regla and len(str(valor).replace('-', '')) > regla['max_digitos']:
                    errores.append(f"El campo {campo} no puede tener más de {regla['max_digitos']} dígitos")
            except ValueError:
                errores.append(f"El campo {campo} debe ser un número entero válido")
                continue
        
        # Validación de máximo de caracteres (debe ir DESPUÉS de las validaciones de formato)
        if 'max_caracteres' in regla:
            if len(str(valor)) > regla['max_caracteres']:
                errores.append(f"El campo {campo} no puede tener más de {regla['max_caracteres']} caracteres")
    
    return errores


log_registros = cargar_log(ruta_log) # Cargar datos del log
...
with st.sidebar: # Panel de conexión
    ...
    if st.session_state.connected:
        ...
        if base_seleccionada:
            ...
            if tabla_seleccionada:
                ...
                # MOVER LA VALIDACIÓN FUERA DEL FORMULARIO
                if submitted:
                    pk_exists = False # Validar PKs antes de insertar
                    error_messages = []
                    ...
                    # Aplicar validaciones adicionales desde el YAML
                    for col, val in zip(columnas, valores):
                        validaciones = aplicar_validaciones(
                            base=base_seleccionada,
                            tabla=tabla_seleccionada,
                            campo=col,
                            valor=val
                        )
                        error_messages.extend(validaciones)
                    
                    if error_messages:
                        # Mostrar todos los errores en un contenedor destacado
                        with st.container():
                            st.error("### Errores de validación - Registro NO guardado")
                            for msg in error_messages:
                                st.write(f"• {msg}")
                            st.warning("🔴 Corrija los errores e intente nuevamente")
                    else:
                ...
                with col2:
                    if pk_columns: # Mostrar último registro (si hay PKs)
                        try:
                        ...
                        st.info("🔑 Claves primarias:") # Mostrar información sobre las PKs
...
with tab2: # 2da pestaña (Registro de Logs)
    st.title("📋 Registro de Actividad")
    st.info("🕒 Historial de los últimos registros agregados:")
    
    for entrada in reversed(log_registros[-20:]): # Mostrar los últimos 20 registros (los más recientes arriba)
